...
### Added
- **Accessibility (WCAG 2.2 AA).** Capture, Liveness, and Validation screens are being brought to
  WCAG 2.2 AA — screen-reader semantics, color contrast, Dynamic Type / font scaling, focus order,
  and minimum tap-target sizes. (Android + iOS)
...
Native SDK milestones prior to the public-registry launch. These were distributed privately; the
entries below summarize the developer- and user-facing capabilities added in each milestone.
